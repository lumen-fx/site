...
def _map_nav(entry, transform):
    """Apply transform to every path string in a nav entry, recursively."""
    if isinstance(entry, str):
        return transform(entry)
    if isinstance(entry, list):
        return [_map_nav(e, transform) for e in entry]
    if isinstance(entry, dict):
        return {k: _map_nav(v, transform) for k, v in entry.items()}
    sys.exit(f"error: unexpected nav entry: {entry!r}")


def _emit_nav(value, indent: str) -> str:
    """Serialize a nav structure back to the TOML style the products use."""
    if isinstance(value, str):
        return '"' + value + '"'
    if isinstance(value, dict):
        (label, child), = value.items()
        return '{"' + label + '" = ' + _emit_nav(child, indent) + "}"
    if isinstance(value, list):
        inner = indent + "  "
        items = ",\n".join(inner + _emit_nav(v, inner) for v in value)
        return "[\n" + items + ",\n" + indent + "]"
...
def unified_nav(lumen_root: Path, candela_root: Path) -> str:
    ...
    lumen_entries = [
        e
        for e in lumen_nav
        if not (isinstance(e, dict) and list(e.values()) == ["/candela/"])
    ]

    docs_host = "https://docs.lumenfx.dev"

    def candela_path(path: str) -> str:
        if path.startswith(docs_host):
            return path[len(docs_host) :] or "/"
        if path.startswith(("http://", "https://", "/")):
            return path
        return "candela/" + path

    candela_entries = [_map_nav(e, candela_path) for e in candela_nav]
    candela_entries.append({"Back to the Lumen docs": "/"})

    nav = lumen_entries + [{"Candela": candela_entries}]
    return "nav = " + _emit_nav(nav, "") + "\n"


def generate_config(target: str, nav_text: str | None = None) -> Path:
    """Write zensical.<target>.toml at the repo root: per-target block, the
    generated nav if the target has one, then the theme."""
